...
export const CONTENT_ENTRIES: ContentEntry[] = [
  ...
  },
  {
    ...
    category: 'section',
    confidence: 'curated',
    description: 'Node lifecycle, churning, slashing, vault security, live operational state, and Maya Protocol node data.',
    body: 'Network security nodes node operator guide node operator setup how to run a THORChain node run THORNode node setup validator node become a node operator Managing THORNodes node lifecycle Whitelisted Standby Ready Active Disabled Standby-only unbond outside vault migration churning slashing slash points bond-principal slashing vault safety Bifrost TSS reader path current-only operational state official halt keys HALTTRADING HALTSIGNING HALTCHURNING HALTWASMGLOBAL StreamingSwapPause HaltMemoless RUNEPoolHaltDeposit memoless halt streaming swap pause RUNEPool deposit halt. Maya Protocol nodes MAYANode Maya validator CACAO bonding Maya network Maya active nodes Maya standby nodes Maya node data Maya Midgard. Use incident records and the TSS deep dive for dated exploit and cryptographic migration wording.',
    tags: ['nodes', 'security', 'slashing', 'churning', 'maya', 'maya-protocol'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [docsPageSource, nodeOperationsSource, nodeLeavingSource, nodeManagingSource, nodeRisksRewardsSource, thornodeBifrostHowItWorksSource, thornodeVaultBehaviorsSource, networkHaltsPageSource, exploitReport2Source, protocolUpgradeV319Source, exploitReportSource],
    nav: true,
    footer: true,
    featured: true,
  },
  {
    ...
    body: 'RUNE settlement asset CLP constant function slip ratio liquidity fee affiliate fee outbound fee incentive pendulum RUNEPool protocol owned liquidity POL live RUNEPool POL current snapshot same provider same height read this snapshot first can I deposit can I withdraw which value matters no yield proof RUNEPool units provider value provider pnl negative pnl negative current_deposit pol.value pol.pnl providers.value providers.pnl reserve.value RUNEPoolDepositMaturityBlocks RUNEPoolMaxReserveBackstop POL enabled pools POL-<Asset> live Mimir fixed pool count is not current scope runepool endpoint trade assets secured assets liquidity units APY economic claim checks current metric claim fee revenue claim tokenomics 425M approximate 500M original cap source divergence live data source health dated source do not claim future yield revenue lift route competitiveness recovery guarantees.',
  ...
  },
  {
    ...
    nextReviewDue: '2026-11-17',
    sources: [adr026DynamicFeesPageSource, feesPageSource, thornameGuidePageSource, liquifyDynamicL1FeesSource, liquifyDynamicL1FeesCurrentSource],
    nav: true,
    footer: true,
    featured: true,
  },
  {
    ...
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [ecosystemPageSource, docsPageSource, devDocsPageSource, liquifyLiveInboundSource, networkHaltsPageSource],
    nav: true,
    footer: true,
    featured: true,
  },
  {
    ...
    body: 'Governance ADR Mimir halt trading halt signing pause LP incidents exploit reports GG20 THORFi unwind TCY recovery source-backed history governance claim checks operational Mimir claim ADR proposal status submit proposal vote proposal proposal voting governance proposal governance vote ADR proposal incident root-cause claim recovery solvency claim community sentiment claim live diagnostics current recovery tracker Recovery State Matrix THORFi debt unwind GG20 exploit recovery current user actions Safe current wording Verify Next Do Not Use This For ADR-028 Accepted v3.19.0 one-time conciliation migration no universal made-whole proof source boundary. Use incident records and security deep dives for exact cryptographic root-cause wording.',
    tags: ['governance', 'incidents', 'mimir', 'adr'],
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [docsPageSource, networkHaltsPageSource, exploitReport2Source, protocolUpgradeV319Source, postRestartSecuritySource, adr028ExploitConciliationSource, exploitReportSource],
    nav: true,
    footer: true,
    featured: true,
  },
  {
    ...
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [liquifyMidgardHealthSource, liquifyMidgardNetworkSource, liquifyMidgardPoolsSource, liquifyMidgardEarningsSource, liquifyThornodeMimirSource, devDocsPageSource],
    nav: true,
    footer: true,
  },
  {
  ...
  },
  {
    ...
    body: 'Deep dives build query data developer integration Midgard API THORNode API inbound_addresses quote swap Mimir source safety CLP constant function liquidity actions LP actions RUNEPool POL protocol owned liquidity runepool endpoint same provider same height provider pnl negative current_deposit add liquidity withdraw liquidity asymmetric withdrawal pool deposits Bifrost TSS churning slashing incentive pendulum RUNE settlement App Layer CosmWasm secured assets WASM controls Savers historical IL protection removed halt Mimir swap lifecycle Mimir halt controls operational controls source warnings absent keys streaming swaps refunds quote expiry refund reasons transaction evidence Midgard THORNode data failover provider freshness current-only source labels pinned snapshots dashboard metrics raw protocol state TCY recovery timeline THORFi unwind Proposal 6 full debt recovery not guaranteed.',
  ...
  },
  {
    ...
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [docsPageSource, devDocsPageSource, swapGuideSource, memosSource, feesPageSource, networkHaltsPageSource, adr026DynamicFeesPageSource, exploitReport2Source],
    footer: true,
  },
  {
    id: 'docs',
  ...
  {
    id: 'deep-dive-clp',
    title: 'Continuous Liquidity Pools (CLP) And The Decentralized Exchange',
    href: '/deep-dives/clp',
    category: 'deep-dive',
    confidence: 'curated',
    description: 'How THORChain\'s constant-function, slip-priced pools support native swaps, and where formula evidence stops before current route, refund, LP, or yield claims.',
    body: 'Continuous Liquidity Pools CLP decentralized exchange DEX constant function AMM constant product style what CLP can prove evidence ladder slip ratio x divided by X plus x liquidity fee output amount RUNE paired pools slippage liquidity providers native cross-chain swaps swap lifecycle quote inbound memo execution outbound refund stale quote min output recommended_min_amount_in quote expiry halt signing trading pool depth APY route availability LP action availability Liquidity Actions RUNEPool Streaming Swaps And Refunds impermanent loss protection removed historical non-claims not route safety not wallet instructions not yield proof. Savers are historical and deprecated.',
    tags: ['clp', 'fees', 'swaps'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
  ...
  },
  {
    ...
    body: 'Liquidity Actions LP actions add liquidity withdraw liquidity LP deposit LP withdrawal pool deposits asymmetric withdrawal PauseLP PauseLPDeposit PauseAsymWithdrawal RUNEPool liquidity units paired address symmetric deposit asymmetric deposit pending liquidity cleanup LiquidityLockUpBlocks liquidity lockup blocks basis points dust threshold memo ADD WITHDRAW Midgard pools LP APY earnings intervals pooled RUNE network diagnostics source warnings impermanent loss protection removed historical not wallet instructions not yield proof not swap halt.',
  ...
  },
  {
    ...
    body: 'RUNEPool POL protocol owned liquidity evidence RUNEPool units provider value provider pnl negative pnl current_deposit can be negative runepool endpoint same provider same height pinned snapshot rune_provider endpoint POL enabled pools live Mimir scope fixed pool count is not current scope POL-<Asset> RUNEPOOLENABLED RUNEPoolDepositMaturityBlocks RUNEPoolMaxReserveBackstop deposit halt withdraw halt current-only accounting source freshness checked height aggregate exposure impermanent loss negative yield non-claims future yield profitability investment suitability route health wallet instructions.',
  ...
  },
  {
    ...
    body: 'Streaming swaps refunds swap refund swap failed quote expiry do not cache quote stale inbound address never cache inbound address re-fetch across churn recommended_min_amount_in dust threshold memo invalid memo too long documented memo handler does not prove feature availability destination address price tolerance liquidity_tolerance_bps min output refund address StreamingSwapPause HaltMemoless affiliate bps quote error expected_amount_out fees slippage_bps total_bps outbound fee route halted signing halted trading halted evidence ladder transaction hash non-claims.',
    tags: ['swaps', 'refunds', 'streaming', 'memos'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [swapGuideSource, memosSource, feesPageSource, networkHaltsPageSource, liquifyLiveInboundSource],
    featured: true,
  },
  {
    ...
    body: 'Incentive Pendulum bonded RUNE pooled capital pooled liquidity rewards node operators liquidity providers security liquidity ratio design target 2:1 bond-to-stake ratio economic model emission schedule reserve APY reward split live metrics Midgard health source mismatch 500M original maximum 425M approximate tokenomics source divergence evidence ladder common misreadings current APY realized yield route quoteability protocol revenue lift not investment suitability network health non-claims.',
  ...
  },
  {
    ...
    category: 'deep-dive',
    confidence: 'curated',
    description: 'How distributed signing protects cross-chain vault keys, what v3.19.x recovered, and why GG20, DKLS/FROST, Paillier, and vault-safety wording remain dated-source claims.',
    body: 'Threshold Signature Scheme TSS Asgard vault node churn GG20 DKLS FROST Schnorr Paillier multi-prime modulus MTA round key-sign failures vault key shares signing ceremony exploit May 2026 solvency checker halt signing KeyVerify compromised vault exclusion pause safety Solana EdDSA not exposed v3.19.0 v3.19.1 trading resumed post-restart future migration chain by chain not deployed current vault safety live evidence.',
    tags: ['tss', 'security', 'vaults'],
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [docsPageSource, asgardTssChurnSource, exploitReport2Source, protocolUpgradeV319Source, postRestartSecuritySource, exploitReportSource],
    featured: true,
  },
  {
    ...
    body: 'Churning node lifecycle Whitelisted Standby Ready Active Disabled churn interval vault rotation migration unbonding Standby-only outside vault migration churn-out criteria slash points low version low bond leave request HALTCHURNING current churn height active set readiness vault migration not proof of unbonding availability.',
    tags: ['nodes', 'churning'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [nodeOperationsSource, nodeLeavingSource, bifrostTssVaultsSource, thornodeVaultBehaviorsSource, nodeRisksRewardsSource, constantsMimirSource],
  },
  {
    ...
    body: 'Slashing economic security bonded RUNE slash points reward reduction bond-principal slashing unauthorized outbound double block signing missed block signing signing observation keygen keysign failures constants Mimir current-only node status operator evidence ladder not every fault burns bond not every incident made whole.',
    tags: ['security', 'slashing'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [nodeRisksRewardsSource, nodeManagingSource, thornodeVaultBehaviorsSource, constantsMimirSource],
  },
  {
    ...
    body: 'Bifrost observer signer external chains inbound transactions Active and Retiring vaults outbound confirmations 67 percent consensus chain reorgs delayed finality dynamic confirmations observation chain clients gas confirmation broadcast inbound_addresses retired inactive vault route execution quote memo outbound refund diagnostics source freshness.',
    tags: ['bifrost', 'observation'],
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [bifrostTssVaultsSource, thornodeStackSource, thornodeBifrostHowItWorksSource, thornodeVaultBehaviorsSource],
  },
  {
    ...
    body: 'App Layer CosmWasm WASM x/wasm Rujira RUJI secured assets secured asset movement trade assets trade accounts source terminology divergence replace Trade Assets replace Trade Accounts contract sandbox vault logic TokenFactory X assets IBC SECURE+ SECURE- memo syntax is not availability App Layer claim checks what this page can prove evidence ladder common misreadings contract app availability interface wallet support HaltSecuredGlobal HaltSecuredDeposit HaltSecuredWithdraw HaltWasmGlobal HaltWasmDeployer HaltWasmCs HaltWasmContract HaltOracle WasmPermissionless WasmMinGasPrice current-only Mimir network diagnostics no global WASM halt scoped deployer checksum contract oracle source warnings wallet listing not safety review absent halt keys not health proof.',
  ...
  },
  {
    ...
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [networkHaltsPageSource, constantsMimirSource, liquifyThornodeMimirSource, liquifyLiveInboundSource, devDocsPageSource],
    featured: true,
  },
  {
    ...
    body: 'RUNE settlement universal settlement asset BTC RUNE ETH internal pool legs base layer pool pairs unified liquidity price discovery security bond liquidity pair common settlement intermediate asset user does not need two transactions route availability live pool depth quote quality non-claims current APY investment value.',
  ...
  },
  {
    ...
    body: 'Savers permanently deprecated Lending permanently deprecated January 4 2025 2025-01-04 THORFi historical single-sided exposure synthetics liabilities TCY archived docs documented memo handler does not prove availability TCY Recovery Timeline Proposal 6 current TCY Mimir controls TCYCLAIMINGHALT TCYSTAKINGHALT TCYSTAKEDISTRIBUTIONHALT TCYUNSTAKINGHALT HALTTCYTRADING evidence ladder common misreadings full recovery not guaranteed non-claims Savers are back TCY exists historical liability numbers are current debt.',
  ...
  },
  {
    ...
    onPageNav: [
      ...
      { id: 'mimir-reference', label: 'Mimir Reference' },
    ],
  },
];
...
export const DEEP_DIVE_READER_PATHS: DeepDiveReaderPath[] = [
  ...
  },
  {
    ...
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [nodeOperationsSource, nodeLeavingSource, bifrostTssVaultsSource, thornodeBifrostHowItWorksSource, thornodeVaultBehaviorsSource, nodeRisksRewardsSource, exploitReport2Source, protocolUpgradeV319Source, networkHaltsSource, constantsMimirSource],
  },
  {
    ...
    searchTerms: ['app layer integrations', 'app layer', 'CosmWasm', 'secured assets', 'trade assets', 'trade accounts', 'replace Trade Assets', 'replace Trade Accounts', 'contract sandbox', 'vault logic', 'SECURE memo syntax', 'WASM halt', 'HaltWasm', 'HaltSecured'],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
  ...
  },
  {
    ...
    searchTerms: ['historical recovery', 'Savers Lending', 'January 4 2025', '2025-01-04', 'permanently deprecated', 'archived memo handler', 'THORFi', 'TCY', 'TCY recovery timeline', 'Proposal 6', 'full debt recovery not guaranteed', 'exploit recovery', 'ADR-028', 'v3.19.0 restart', 'v3.19.1 patch', 'security incident'],
    confidence: 'historical',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [archivedSaversSource, archivedLendingSource, thorfiUnwindSource, tcyGuideSource, tokenomicsSource, exploitReportSource, exploitReport2Source, protocolUpgradeV319Source, postRestartSecuritySource],
  },
];
...
export const TASK_INTENT_GUIDES: TaskIntentGuide[] = [
  ...
  },
  {
    ...
    searchTerms: [
      ...
      'documented feature versus available action',
      'source terminology conflict',
    ],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
  ...
  },
  {
    ...
    searchTerms: [
      ...
      'RUNE market cap',
      'RUNE exchange float',
      'RUNE 500M original cap',
      'RUNE 425M and burning',
      'RUNE supply source divergence',
    ],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [docsSource, tokenomicsSource, economicModelSource, devDocsSource, midgardNetworkSource],
  },
  {
    id: 'rune-actions',
  ...
  },
  {
    ...
    searchTerms: ['RUNEPool', 'RUNEPool guide', 'RUNEPool live', 'RUNEPool current snapshot', 'POL', 'protocol owned liquidity', 'RUNEPool PnL', 'provider PnL', 'negative PnL', 'negative current_deposit', 'current_deposit can be negative', 'same provider same height', 'RUNEPool value', 'RUNEPool deposit halt', 'RUNEPool withdrawal', 'POL enabled pools', 'fixed pool count', 'yield proof'],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
  ...
  },
  {
    ...
    href: '/deep-dives/app-layer#evidence-ladder',
    description: 'Start with the App Layer explainer, preserve the documented Trade Assets versus Trade Accounts terminology conflict, then verify live WASM, secured-asset, trade-account, oracle, signing, trading, and chain controls.',
    searchTerms: ['app layer', 'App Layer claim checks', 'CosmWasm', 'secured asset', 'secured assets', 'secured asset movement', 'secured asset redemption', 'secured asset deposit available', 'secured asset withdrawal halted', 'can I deposit secured assets', 'trade asset', 'trade assets', 'trade account', 'trade accounts', 'replace Trade Assets', 'replace Trade Accounts', 'source terminology conflict', 'trade account enabled', 'trade account deposit halted', 'trade-account flow', 'arbitrage flow', 'WASM contract', 'contract availability', 'contract halted', 'contract sandbox vault logic', 'WASM app live', 'WASM deployer halted', 'app layer route available', 'is app layer available now', 'interface wallet support', 'Rujira', 'RUJI', 'HaltWasmGlobal', 'HaltWasmContract', 'HaltWasmDeployer', 'HaltSecuredDeposit', 'HaltSecuredWithdraw', 'TradeAccountsEnabled', 'WasmPermissionless', 'SECURE+', 'SECURE-', 'SECURE memo syntax does not prove availability'],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [appLayerSource, cosmWasmSource, securedAssetsDevSource, networkHaltsSource, constantsMimirSource],
  },
  {
    ...
    href: '/deep-dives/tss',
    description: 'Start with the TSS explainer and dated incident/restart reports: v3.19.x recovered normal signing, while DKLS/FROST migration and present vault safety still require current proof.',
    searchTerms: [
      'TSS security',
      'TSS safety',
      'vault safety',
    ...
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [docsSource, asgardTssChurnSource, exploitReport2Source, protocolUpgradeV319Source, postRestartSecuritySource, exploitReportSource],
  },
  {
  ...
  },
  {
    ...
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [nodeOperationsSource, nodeLeavingSource, nodeManagingSource, nodeRisksRewardsSource, networkHaltsSource, thornodeMimirSource],
  },
  {
  ...
  },
  {
    ...
    href: '/deep-dives/tcy-recovery-timeline#what-to-check-now',
    description: 'Use the dated timeline to separate THORFi/TCY from accepted ADR-028 exploit conciliation; use Current TCY Controls before claim, stake, distribution, unstake, claim-swap, or trading availability claims.',
    searchTerms: [
      'TCY',
      'recovery',
      'THORFi',
      ...
      'ADR-028',
      'ADR-028 Accepted',
      'exploit conciliation migration',
      'recovery token',
    ...
    reviewedAt: '2026-07-13',
    nextReviewDue: '2026-11-17',
    sources: [thorfiUnwindSource, tokenomicsSource, archivedSource, tcyGuideSource, exploitReportSource, exploitReport2Source, protocolUpgradeV319Source, adr028ExploitConciliationSource],
  },
  {
  ...
  },
  {
    ...
    searchTerms: [
      ...
      'multi-protocol route provider',
      'THORSwap route protocol',
    ],
    confidence: 'curated',
    reviewedAt: '2026-07-14',
    nextReviewDue: '2026-11-17',
    sources: [docsSource, devDocsSource, thorSwapDocsSource],
  },
];
...
export const DEEP_DIVE_TOC: Record<string, DeepDiveTocItem[]> = {
  ...
  'deep-dive-governance-comprehensive': [
    ...
    tocItem('Governance Process'),
    tocItem('Mimir Reference'),
  ],
};
