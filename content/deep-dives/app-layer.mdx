...
Match the claim to its control family: contracts need current Network diagnostics plus deployer/checksum evidence; secured assets need global or chain-scoped deposit/withdraw halts; trade-account flows need `TradeAccountsEnabled` and related controls; wallets and interfaces also need implementation review. Design docs alone do not prove that an action works now.
...
A listed wallet or app is not a safety review. Static documentation cannot prove current capacity, revenue terms, redemption timing, or interface correctness; use live THORNode/Mimir evidence for present-tense claims.
