...
- `TradeAccountsEnabled`: whether the trade-account feature is enabled.
- `TradeAccountsDepositEnabled`: whether trade-account deposits are enabled globally.
- `HaltTradeDeposit-<CHAIN>`: the activation height for halting trade-account deposits on exactly one recognized chain.
- `HaltTradeWithdraw-<CHAIN>`: the activation height for halting trade-account withdrawals on exactly one recognized chain.
