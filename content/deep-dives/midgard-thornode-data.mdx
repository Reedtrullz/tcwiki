...
This page can support source-selection claims:
...
| `/network` source posture | THORNode operational evidence: Mimirs, inbound fields, quote probes | `/network#network-diagnostics` | A degraded source on one page should not be quietly converted to a healthy number on another. |
| `/stats` source posture | Midgard dashboard metrics with source health and coverage labels | `/stats` | Interpreting liquidity, earnings, APY, or node-count figures without provider health labels. |
| `/dynamic-fees` source posture | THORNode dynamic-fee endpoints and Mimirs for current ADR-026 state | `/dynamic-fees#dynamic-fees-live` | Claims about durable revenue lift from current-only accumulator records. |
| `/docs` source guidance | Source family selection guidance | `/docs` | Choosing the wrong source family for the claim type. |
| `/api/ready` source posture | Live source usability check for the app | `/api/ready` | Confusing a degraded readiness response with protocol failure. |
