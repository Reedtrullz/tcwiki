...
For the canonical dashboard evidence rules — including never rendering missing data as `0`, keeping `null` and `unavailable` distinct, and letting malformed provider data fail that provider — see [Dashboard Evidence Rules](/deep-dives/midgard-thornode-data#dashboard-evidence-rules) in the Midgard and THORNode Data guide.

Builder-specific guidance for upstream failure:
