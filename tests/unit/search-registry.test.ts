...
describe('SEARCH_DOCUMENTS', () => {
  it('includes deep-dive bodies and curated records', () => {
    ...
    expect(docsMatching('permanently deprecated on January 4, 2025').some((doc) => doc.slug === '/deep-dives/savers')).toBe(true);
    expect(docsMatching('recovery token framing').some((doc) => doc.slug === '/deep-dives/savers')).toBe(true);
    ...
    expect(docsMatching('RUNE investment value').some((doc) => doc.slug === '/deep-dives/rune-settlement')).toBe(true);
    expect(docsMatching('What CLP Can Prove').some((doc) => doc.slug === '/deep-dives/clp')).toBe(true);
    expect(docsMatching('A formula alone is only partial evidence for a present tense action').some((doc) => doc.slug === '/deep-dives/clp')).toBe(true);
    expect(docsMatching('does not prove that a current route is quoteable, cheap, available, or safe').some((doc) => doc.slug === '/deep-dives/clp')).toBe(true);
    ...
    expect(docsMatching('historical metrics do not guarantee future yield').some((doc) => doc.slug === '/deep-dives/liquidity-actions')).toBe(true);
    ...
    expect(docsMatching('Current signing and vault safety claims still need live diagnostics').some((doc) => doc.slug === '/deep-dives/tss')).toBe(true);
  ...
  it('indexes source-aware glossary terms', () => {
    ...
    expect(docsMatching('does not sign, broadcast, or prove settlement').some((doc) => doc.id === 'glossary:quote')).toBe(true);
    expect(docsMatching('valid for only ten minutes').some((doc) => doc.id === 'glossary:quote-expiry')).toBe(true);
    expect(docsMatching('1e8 units of the inbound asset').some((doc) => doc.id === 'glossary:recommended-min-amount-in')).toBe(true);
    expect(docsMatching('amounts below it are ignored').some((doc) => doc.id === 'glossary:dust-threshold')).toBe(true);
    expect(docsMatching('valid for the inbound asset chain').some((doc) => doc.id === 'glossary:refund-address')).toBe(true);
    expect(docsMatching('market swaps auto-stream by default').some((doc) => doc.id === 'glossary:streaming-swap')).toBe(true);
    expect(docsMatching('fee-aware minimum output').some((doc) => doc.id === 'glossary:liquidity-tolerance-bps')).toBe(true);
  ...
  it('indexes task intent guides for common reader jobs', () => {
    ...
    expect(docsMatching('WASM contract').some((doc) => doc.id === 'task:app-layer-and-secured-assets' && doc.href === '/deep-dives/app-layer#evidence-ladder' && doc.slug === '/deep-dives/app-layer')).toBe(true);
